...
type RestOf<T extends IShoppingCart | NewShoppingCart> = Omit<T, 'purchaseDate'> & {
...
export class ShoppingCartService {
  ...
  protected convertDateFromClient<T extends IShoppingCart | NewShoppingCart | PartialUpdateShoppingCart>(shoppingCart: T): RestOf<T> {
    return {
      ...shoppingCart,
      purchaseDate: shoppingCart.purchaseDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restShoppingCart: RestShoppingCart): IShoppingCart {
    return {
      ...restShoppingCart,
      purchaseDate: restShoppingCart.purchaseDate ? dayjs(restShoppingCart.purchaseDate) : undefined,
    };
  }
